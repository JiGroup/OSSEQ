...
p1SamFile = open(p1Sam,'r')
p2SamFile = open(p2Sam,'r')
p1SamFileFiltered = p1Sam.replace('sam', 'filtered.sam')

addHeader = "samtools view -HS "+p1Sam+" > "+p1SamFileFiltered
os.system(addHeader)
p1SamFiltered =  open(p1SamFileFiltered, 'a')
...
while True:
    ...
    def insertsize():
        dist = 0
        if flag(p1Flag)[6] == 0 and flag(p1Flag)[6] == 1: #read 1 is on positive strand, read 2 on reverse/negative strand
    ...
    if insertsize() > 0 or insertsize() < 1000:
        print >> p1SamFiltered, p1Line[:-1] 
