...
# Varscan_combine.py takes all varscan files (based on ".vcf" extension) in the directory, combines the variants from each file into 1 combined table
...
headers = [x for x in os.listdir('.') if x.endswith('.vcf')] #List the files ending with '.out' in the directory
...
#Iterate through the vcf files, 2 files are created in this step:
